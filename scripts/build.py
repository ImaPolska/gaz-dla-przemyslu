...
def footer_data():
    groups = [
        ...
        ("Narzędzia", [("Wgraj fakturę","/wgraj-fakture/"),("Kalkulator wypowiedzenia","/kontakt/"),("Analiza umowy","/kontakt/"),("Dla agentów AI","/kontakt/")]),
        ("Wiedza", [("Komentarz rynkowy","/wiedza/"),("Ceny i rynek","/wiedza/"),("Zmiana sprzedawcy","/wiedza/"),("Umowy i wypowiedzenia","/wiedza/")]),
        ("Firma", [("O nas","/kontakt/"),("Dokumenty","/kontakt/"),("Kontakt","/kontakt/"),("Dla doradców","/kontakt/")]),
    ]
    data = []
    ...
    data.append(block("paragraph",'<p><a href="/polityka-prywatnosci/">Polityka prywatności</a> · Regulamin: [[ ]] · Informacja o plikach cookie: [[slot CMP: etap 3]]</p>') + '<!-- wp:block {"ref":102} /-->\n' + block("paragraph",'<p>Paleta robocza, logo i kolory firmowe: [[dostarczy zleceniodawca]]. Prototyp etapu 1.</p>'))
    return data
    ...
    return [
        {"slug":"gdp-main","locations":["menu_1","menu_mobile"],"items":[
            ...
            {"title":"Narzędzia","url":"/wgraj-fakture/","children":[
                ...
                {"title":"Kalkulator wypowiedzenia","url":"/kontakt/"},
                {"title":"Analiza umowy","url":"/kontakt/"},
                {"title":"Dla agentów AI","url":"/kontakt/"}]},
            {"title":"Wiedza","url":"/wiedza/"},
            {"title":"Dla doradców","url":"/kontakt/"},
            {"title":"O nas","url":"/kontakt/"},
            {"title":"Kontakt","url":"/kontakt/"}]},
        {"slug":"gdp-legal","locations":["footer"],"items":[{"title":"Polityka prywatności","url":"/polityka-prywatnosci/"}]}
    ]
...
"""
    ...
    starter = '<!-- wp:group {"metadata":{"name":"Sekcja produktu"},"templateLock":"contentOnly","className":"gdp-section gdp-product-content"} -->\n<div class="wp-block-group gdp-section gdp-product-content"><!-- wp:heading --><h2 class="wp-block-heading">[[nagłówek sekcji]]</h2><!-- /wp:heading --><!-- wp:paragraph --><p>[[treść sekcji]]</p><!-- /wp:paragraph --></div><!-- /wp:group -->'
    for slug, title in [("hero-produkt","Otwarcie produktu"),("jak-to-dziala","Jak to działa"),("dla-kogo","Dla kogo"),("ryzyka-i-ograniczenia","Ryzyka i ograniczenia"),("przyklad-liczbowy","Przykład liczbowy")]:
    ...
    for item in mods["header_placements"]["sections"][0]["items"]:
        if item["id"]=="button": item["values"]["header_button_link"]="/kontakt/"
    dump(ROOT / f"config/theme_mods_{direction}.json", mods)
    bundle = ROOT / f"dist/{direction}"
    ...
    blueprint = {
        ...
        "meta":{"title":f"Gaz dla Przemysłu · P1.1 · {direction}: {t['name']}","author":"agent","description":"Prototyp bez wtyczek. Paleta robocza. Brak wysyłania danych."},
    ...
    dump(ROOT / f"blueprints/{direction}.json", blueprint)
    archive(bundle, ROOT / f"dist/etap1-{direction}.zip")
    ...
    testfolder = ROOT / f".runtime/test-{direction}"
    ...
    return {"direction":direction,"wxr_sha256":hashlib.sha256((bundle/"site.wxr").read_bytes()).hexdigest(),"bundle_sha256":hashlib.sha256((ROOT/f"dist/etap1-{direction}.zip").read_bytes()).hexdigest()}

if __name__ == "__main__":
    p=argparse.ArgumentParser()
    p.add_argument("--active",choices=["A","B","C"],default="A")
    args=p.parse_args()
    subprocess.run(["python3",str(ROOT/"scripts/build-wxr.py")],check=True)
    tokens=json.loads((ROOT/"config/tokens.json").read_text())
    reports=[build(d,tokens, d==args.active) for d in ["A","B","C"]]
