...
export async function start(browser) {
  ...
  const paths=[...new Set([...source.items.filter(i=>['page','post'].includes(i.post_type)).map(i=>i.public_path),
    '/category/zmiana-sprzedawcy/','/category/umowy-i-wypowiedzenia/','/category/ceny-i-rynek/','/category/biometan-i-raportowanie/','/category/sprzedaz-rezerwowa/','/komentarz-rynkowy/','/nieistniejacy-adres-p12/','/?s=sprzedawca','/?s=gdpbraktestowegoslowa'])];
...
export async function routes(state,start=0,count=12) {
  for(const path of state.paths.slice(start,start+count)){
    ...
    const headingJumps=await state.page.locator('#main').evaluate(e=>{let prev=0;const bad=[];for(const h of e.querySelectorAll('h1,h2,h3,h4,h5,h6')){const n=+h.tagName[1];if(prev&&n>prev+1)bad.push(h.textContent);prev=n;}return bad;});
    state.report.urls.push({path,status:response.status(),expected:path.includes('nieistniejacy')?404:200,h1,headingJumps,finalURL:state.page.url(),overflow:await state.page.evaluate(()=>document.documentElement.scrollWidth>innerWidth)});
  }
  await state.save();
  console.log({checked:state.report.urls.length,issues:state.report.urls.filter(r=>r.status!==r.expected||r.h1!==1||r.overflow||r.headingJumps.length)});
}
...
export async function interactions(state){
  ...
  r.skipLink=await page.evaluate(()=>({text:document.activeElement.textContent,href:document.activeElement.getAttribute('href')}));
  await page.keyboard.press('Enter');
  r.skipLink.focusedMain=await page.evaluate(()=>document.activeElement.id==='main');
  await page.goto(origin+'/',{waitUntil:'networkidle'});
  ...
  await page.locator('#offcanvas').getByRole('button',{name:'Expand dropdown menu',exact:true}).first().click();
  await page.waitForTimeout(300);
  r.menuExpandedLinks=await page.locator('#offcanvas a:visible').count();
  await page.locator('#offcanvas').getByRole('link',{name:'Cena stała',exact:true}).click();
  await page.waitForURL('**/oferta/cena-stala/');
  r.mobileDestination=page.url();
  await page.locator('[data-id="trigger"]:visible').first().click();
  await page.waitForTimeout(450);
  await page.keyboard.press('Escape');
  await page.waitForTimeout(400);
  r.menuClosedAfterEscape=!(await page.locator('#offcanvas').isVisible());
